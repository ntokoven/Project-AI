...
class MINE(nn.Module):
    ...
    def lower_bound(self, x, y, method='kl', step=2, ema=0, target=True):
        ...
        if method == 'kl':
            ...
            ema_normalization = (1 / ema) * torch.mean(torch.exp(
                T_marginal)).detach()  # multiplying by detached mean to compensate the denominator after derivation of log
...
class TrackMI(nn.Module):
    def __init__(self, args):
        ...
        for layer in ['maxP1', 'maxP2', 'relu3', 'sm1']:
            ...
            self.mineList[layer + 'T'] = MINE(self.dimensions['target'], self.dimensions[layer], self.args,
    ...
    def trainMine(self, trainLoader, mine_epochs, batch_size, plot=False, convNet=None, target=False, \
        ...
        for mine_epoch in range(mine_epochs):
            ...
            loss_list.append(-loss_per_epoch.detach().item() / len(
            ...
            print('Epoch MINE: %s. Layer: %s. Target: %s. Lowerbound: %s' % (
            mine_epoch, layer, target, -loss_per_epoch.detach().cpu().numpy() / len(train_loader)))
        if plot:
            # Plot
    ...
    def run(self, mine_path=None, save=True, mine_method='kl'):
        if len(self.args.conv_path) == 0:
            ...
            torch.save(self.convN.state_dict(), 'ConvNet_models/convN_%s_%s_%s' % (
            self.args.epochs, self.args.batch_size, str(self.args.lr).replace('.', '')))
        else:
        ...
        for layer in optim_layers:
            ...
            self.mi_values[layer] = self.trainMine(self.mine_train_loader, self.mine_epochs, self.mine_batch_size,
            ...
            if save == True:
                ...
                if not os.path.exists(self.args.mine_path + '/mine_model_%s_%s_%s_%s' % (
                self.args.epochs, self.args.mine_epochs, self.args.mine_batch_size,
                str(self.args.mine_lr).replace('.', ''))):
                    os.makedirs(self.args.mine_path + '/mine_model_%s_%s_%s_%s' % (
                    self.args.epochs, self.args.mine_epochs, self.args.mine_batch_size,
                    str(self.args.mine_lr).replace('.', '')))
                path = self.args.mine_path + '/mine_model_%s_%s_%s_%s' % (
                self.args.epochs, self.args.mine_epochs, self.args.mine_batch_size,
                str(self.args.mine_lr).replace('.', ''))
...
def write_results(results, args):
    ...
    filename = 'MINE_results/mine_values_dict_%s_%s_%s_%s.pickle' % (
    args.comment, args.mine_epochs, args.mine_batch_size, str(args.mine_lr).replace('.', ''))

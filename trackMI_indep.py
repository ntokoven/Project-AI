...
class TrackMI():
    def __init__(self, args):
        ...
        for layer in ['maxP1', 'maxP2', 'relu3', 'sm1']:
            self.mineList[layer] = MINE(self.dimensions['input'], self.dimensions[layer])
            self.mineList[layer + 'T'] = MINE(self.dimensions['target'], self.dimensions[layer])
    ...
    def run(self, mine_path=None, save=False):
        ...
        for epoch in range(self.args.epochs):
            ...
            for layer in ['maxP1','maxP2','relu3','sm1']:
                # self.mineList[layer] =
                self.mi_values[layer].append(
                    self.trainMine(self.train_loader, self.mine_epochs, self.batch_size, plot=False,
                                   convNet=self.convN.model, mineMod=self.mineList[layer], target=False, layer=layer))
                # self.mineList[layer+'T'] =
                self.mi_values[layer + 'T'].append(
                    self.trainMine(self.train_loader, self.mine_epochs, self.batch_size, plot=False,
                                   convNet=self.convN.model, mineMod=self.mineList[layer + 'T'], target=True,
                                   layer=layer))
                '''
...
def main():
    ...
    parser.add_argument('--mine-epochs', type=int, default=1, metavar='N',  # set to default 100
                        help='number of epochs to train MINE (default: 10)')
    ...
    with open('mi_values_indep_dict.pickle', 'wb') as handle:
